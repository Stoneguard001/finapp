...
// Random day within a month, optionally clamped to a max
...
async function main() {
  const outFile = resolve(process.argv[2] ?? 'sample.sqlite')
  ...
  // Apply schema + seed
  db.run(readFileSync(join(root, 'src/db/schema.sql'),      'utf8'))
  db.run(readFileSync(join(root, 'src/db/schema-seed.sql'), 'utf8'))
  ...
  const txCount = db.exec('SELECT COUNT(*) FROM transactions')[0].values[0][0]
  const data    = db.export()
  writeFileSync(outFile, Buffer.from(data))
  db.close()

  console.log(`✓ ${txCount} transactions across 3 accounts → ${outFile}`)
}
