  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <input
          className="bg-transparent outline-none text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 flex-1 min-w-[80px]"
        ...
        <div className="absolute z-50 top-full mt-1 left-0 right-0 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl overflow-hidden">
          {filtered.map(t => (
            <button
              key={t.id}
              type="button"
              className="w-full flex items-center gap-2 px-3 py-2 hover:bg-slate-100 dark:hover:bg-slate-700 text-left text-sm text-slate-800 dark:text-slate-200"
          ...
          {canCreate && (
            <div className={`px-3 py-2 ${filtered.length > 0 ? 'border-t border-slate-200 dark:border-slate-700' : ''}`}>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-xs text-slate-600 dark:text-slate-400">Create</span>
                <span className="text-sm text-slate-800 dark:text-slate-200 font-medium truncate">"{input.trim()}"</span>
