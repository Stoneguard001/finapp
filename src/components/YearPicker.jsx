...
export default function YearPicker({ year, onChange, maxYear }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <button onClick={() => onChange(year - 1)} className="btn-ghost p-2.5" title="Previous year">
        <ChevronLeft size={18} />
      </button>
      <span className="text-xl font-semibold text-slate-900 dark:text-slate-100 w-16 text-center select-none">
        {year}
      </span>
      <button onClick={() => onChange(year + 1)} className="btn-ghost p-2.5" disabled={atMax} title="Next year">
